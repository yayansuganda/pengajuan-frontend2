...
export const JenisPembiayaanList: React.FC = () => {
    ...
    const { user } = useAuth();
    const [data, setData] = useState<JenisPembiayaan[]>([]);
    ...
    const [editingItem, setEditingItem] = useState<JenisPembiayaan | null>(null);
    ...
    const openEditModal = (item: JenisPembiayaan) => {
        setEditingItem(item);
        setFormData({
            name: item.name,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (editingItem) {
                await repository.update(editingItem.id, formData as UpdateJenisPembiayaanDTO);
                hideLoading();
                await showSuccess('Data berhasil diupdate');
            } else {
                await repository.create(formData as CreateJenisPembiayaanDTO);
                hideLoading();
